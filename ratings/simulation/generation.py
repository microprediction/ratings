...
def mock_data(n_contestants=100, n_days =1000, min_contest_size:int=6, max_contest_size:int=16,
              performance_std=3, field_std=2, market_std=0.5, show_progress=True):
    ...
    :param performance_std                      variation in performance of contestant "on the day"
    ...
    :return:
    """
    ability_data = mock_ability(n_contestants=n_contestants, n_days =n_days, performance_std=performance_std)
    ...
    for day_ndx, row in enumerate(ability_data):
        ...
        for i,ability in enumerate(row):
            if not np.isnan(ability):
                performance = ability + np.random.randn() * performance_std
        ...
        if any(sorted_candidates):
            ...
            for contest in random_chunk(sorted_candidates, min_chunk=min_contest_size, max_chunk=max_contest_size):
                 ...
                 true_dividends = std_ability_implied_dividends(ability=centered_relative_abilities,scale=performance_std)
                 noisy_relative_abilities = [ ra+np.random.randn()*market_std for ra in centered_relative_abilities ]
                 market_dividends = std_ability_implied_dividends(ability=noisy_relative_abilities, scale=performance_std)
